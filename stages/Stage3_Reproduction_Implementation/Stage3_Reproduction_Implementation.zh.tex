...
不变，因此不同划分不会改变收敛动力学；划分影响的是有多少消息需要跨 worker
边界传输。
...
Kernighan--Lin 细化方法取得了最低的平均割值和最低的平均跨 worker 通信量。在
...
一致性实验也有一个限制：它测量跨 worker 通信量，但没有改变底层通信拓扑。
